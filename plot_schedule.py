...
Written by: Helga Denes (denes@astron.nl)    
...
#data['altazframe'] = data.apply (lambda row: AltAz(location=WSRT, obstime=row['timespan']), axis=1) #old format
data['my_obj'] = data.apply (lambda row: SkyCoord(ra=row['ra'], dec=row['dec'], unit=(u.hourangle, u.deg)), axis=1)
#data['my_obj_altaz'] = data.apply (lambda row: row['my_obj'].transform_to(row['altazframe']), axis=1)  #old format
data['my_obj_altaz'] = data.apply (lambda row: row['my_obj'].transform_to(AltAz(location=WSRT, obstime=row['timespan'])), axis=1)
